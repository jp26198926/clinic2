...
class Data_product_model extends CI_Model
{
	...
	function search($search = "", $active_only = 0, $limit=0)
	{
		...
		$this->db->select(
			"x.*,
			 m.code as uom_code, m.name as uom_name,
			 c.category,
		...
		$this->db->join("categories c", "c.id=x.category_id", "left");
		$this->db->join("product_status s", "s.id=x.status_id", "left");
		...
		if ($search) {
			$this->db->where("CONCAT_WS(' ',x.code, x.name,
											m.code, m.name,
											c.category,
