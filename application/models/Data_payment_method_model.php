...
class Data_payment_method_model extends CI_Model
{
	...
	function search($search = "", $active_only = 0)
	{
		...
		$this->db->from("payment_methods x");
		...
		$this->db->join("payment_method_status s", "s.id=x.status_id", "left");
		$this->db->order_by('x.payment_method');
		...
		if ($search) {
			$this->db->where("CONCAT_WS(' ',x.payment_method,s.status) LIKE '%{$search}%'");
		}
