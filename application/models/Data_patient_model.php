...
class Data_patient_model extends CI_Model
{
	private $tablename = "patients";

	function search($search = "", $active_only = 0)
	{
		$this->db->select(
			"x.*,
			 CONCAT('P',LPAD(x.id,5,'0')) as code,
			 CONCAT(x.lastname,', ', x.firstname,' ', x.middlename) as patient,
			 g.gender, c.civil as civil_status,
		...
		$this->db->join("patient_status s", "s.id=x.status_id", "left");
		$this->db->join("genders g","g.id=x.gender_id","left");
		$this->db->join("civils c", "c.id=x.civil_id", "left");
		$this->db->order_by('x.lastname, x.firstname, x.middlename');
		...
		if ($search) {
			$this->db->where("CONCAT_WS(' ',x.lastname,x.firstname,x.middlename,
											x.address,x.contact_no,x.address,
											CONCAT('P',LPAD(x.id,5,'0'),
											g.gender, c.civil,
