...
class Data_result_set_model extends CI_Model
{
	private $tablename = "result_sets";

	function search($search = "", $active_only = 0)
	{
		$this->db->select(
			"x.*,
			 p.code as product_code, p.name as product_name, p.category_id,
			 c.category,
		...
		$this->db->join("result_set_status s", "s.id=x.status_id", "left");
		$this->db->order_by('c.category, p.code, p.name');
		...
		if ($search) {
			$this->db->where("CONCAT_WS(' ',c.category, p.code, p.name,
											x.result,
											x.reference,
											x.unit,
