...
class Data_trans_type_model extends CI_Model
{
	private $tablename = "trans_types";
	...
	function search($search = "", $active_only = 0)
	{
		...
		$this->db->join("trans_type_status s", "s.id=x.status_id", "left");
		$this->db->order_by('x.trans_type');
		...
		if ($search) {
			$this->db->where("CONCAT_WS(' ',x.trans_type,s.status) LIKE '%{$search}%'");
		}
