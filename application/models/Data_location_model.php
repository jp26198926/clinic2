...
class Data_location_model extends CI_Model
{
	...
	function search($search = "", $active_only = 0)
	{
		...
		$this->db->join("location_status s", "s.id=x.status_id", "left");
		$this->db->order_by('x.location');
		...
		if ($search) {
			$this->db->where("CONCAT_WS(' ',x.location,s.status) LIKE '%{$search}%'");
		}
