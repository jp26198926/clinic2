...
class Data_category_model extends CI_Model
{
	...
	function search($search = "", $active_only = 0)
	{
		...
		$this->db->join("category_status s", "s.id=x.status_id", "left");
		$this->db->order_by('x.category');
		...
		if ($search) {
			$this->db->where("CONCAT_WS(' ',x.category,s.status) LIKE '%{$search}%'");
		}
