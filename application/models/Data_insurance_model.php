...
class Data_insurance_model extends CI_Model
{
	private $tablename = "insurances";
	...
	function search($search = "", $active_only = 0)
	{
		...
		$this->db->select(
			"x.*,
			 v.value_type,
			 v2.value_type as commission_type,
		...
		$this->db->join("insurance_status s", "s.id=x.status_id", "left");
		$this->db->join("insurance_value_types v", "v.id=x.value_type_id", "left");
		$this->db->join("insurance_value_types v2", "v2.id=x.commission_type_id", "left");
		$this->db->order_by('x.name');
