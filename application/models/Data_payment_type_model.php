...
class Data_payment_type_model extends CI_Model
{
	private $tablename = "payment_types";
	...
	function search($search = "", $active_only = 0)
	{
		...
		$this->db->join("payment_type_status s", "s.id=x.status_id", "left");
		$this->db->order_by('x.payment_type');
		...
		if ($search) {
			$this->db->where("CONCAT_WS(' ',x.payment_type,s.status) LIKE '%{$search}%'");
		}
