...
class Data_client_model extends CI_Model
{
	...
	function search($search = "", $active_only = 0)
	{
		...
		$this->db->join("client_status s", "s.id=x.status_id", "left");
		...
		if ($search) {
			$this->db->where("CONCAT_WS(' ',x.name,x.address,x.phone,s.status) LIKE '%{$search}%'");
		}
