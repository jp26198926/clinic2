...
class Accounting_invoice_charges extends CI_Controller
{
	...
	protected $role_id;
	protected $module = "accounting_invoice_charges";
	protected $module_description = "Invoice Charges";
	protected $page_name = "Invoice Charges";
	protected $parent_menu = "Accounting";
	protected $uid = 0;
	protected $uname;
	protected $ufullname;
	protected $app_code;
		...
		try {
			...
			if ($ad) {
				...
				if (!isset($this->session->userdata[$this->prefix . '_logged_in'])) {
					...
					$this->load->model("accounting_invoice_charges_model", "main_model");
	...
	function index()
	{
		...
		if ($this->cf->is_allowed_module($this->module, $prefix)) {
			...
			$this->load->view('accounting_invoice_charges/index', $data);
		} else {
			$this->session->set_userdata($this->prefix . '_to_page', current_url());
			redirect(base_url() . 'authentication');
		}
	...
	{
		try {
			$result = $this->main_model->search("", array(2,3));
	...
	function search_row()
	{
		...
		if ($id) {
			try {
				$info = $this->main_model->search_row($id);
				if ($info) {
	...
	function complete()
	{
		...
		if ($transaction_id) {
			try {
				$save = $this->main_model->complete($transaction_id);
				if ($save["proceed"]===true) {
					try {
						$result["data"] = $this->main_model->search_row($transaction_id);
						$result["success"] = true;
					} catch (Exception $ex) {
