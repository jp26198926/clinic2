...
class Data_insurance extends CI_Controller
{
	...
	protected $role_id;
	protected $module = "data_insurance";
	protected $module_description = "Insurance";
	protected $page_name = "Insurance";
		...
		try {
			...
			if ($ad) {
				...
				if (!isset($this->session->userdata[$this->prefix . '_logged_in'])) {
					...
					$this->load->model('data_insurance_model', 'main_model');
					$this->load->model('data_insurance_status_model');
					$this->load->model('data_insurance_value_type_model');
				}
			}
		} catch (Exception $ex) {
			$this->session->set_userdata($this->prefix . '_to_page', current_url());
	...
	function index()
	{
		...
		if ($this->cf->is_allowed_module($this->module, $prefix)) {
			...
			$data['status'] = $this->data_insurance_status_model->search("");
			$data['value_types'] = $this->data_insurance_value_type_model->search("");
			$this->load->view('data_insurance/index', $data);
		} else {
			$this->session->set_userdata($this->prefix . '_to_page', current_url());
			redirect(base_url() . 'authentication');
		}
