...
class Data_payment_type extends CI_Controller
{
	...
	protected $role_id;
	protected $module = "data_payment_type";
	protected $module_description = "Payment Type";
	protected $page_name = "Payment Type";
		...
		try {
			...
			if ($ad) {
				...
				if (!isset($this->session->userdata[$this->prefix . '_logged_in'])) {
					...
					$this->load->model('data_payment_type_model', 'main_model');
					$this->load->model('data_payment_type_status_model');
				}
			}
		} catch (Exception $ex) {
			$this->session->set_userdata($this->prefix . '_to_page', current_url());
	...
	function index()
	{
		...
		if ($this->cf->is_allowed_module($this->module, $prefix)) {
			...
			$data['status'] = $this->data_payment_type_status_model->search("");
			$this->load->view('data_payment_type/index', $data);
		} else {
			$this->session->set_userdata($this->prefix . '_to_page', current_url());
			redirect(base_url() . 'authentication');
		}
