...
class Data_package extends CI_Controller
{
	...
	protected $role_id;
	protected $module = "data_package";
	protected $module_description = "Package";
	protected $page_name = "Package";
		...
		try {
			...
			if ($ad) {
				...
				if (!isset($this->session->userdata[$this->prefix . '_logged_in'])) {
					...
					$this->load->model('data_package_model', 'main_model');
					$this->load->model('data_package_status_model');
				}
			}
		} catch (Exception $ex) {
			$this->session->set_userdata($this->prefix . '_to_page', current_url());
	...
	function index()
	{
		...
		if ($this->cf->is_allowed_module($this->module, $prefix)) {
			...
			$data['status'] = $this->data_package_status_model->search("");
			$this->load->view('data_package/index', $data);
		} else {
			$this->session->set_userdata($this->prefix . '_to_page', current_url());
			redirect(base_url() . 'authentication');
		}
