...
class Data_payment_method extends CI_Controller
{
	...
	protected $role_id;
	protected $module = "data_payment_method";
	protected $module_description = "Payment Method";
	protected $page_name = "Payment Method";
		...
		try {
			...
			if ($ad) {
				...
				if (!isset($this->session->userdata[$this->prefix . '_logged_in'])) {
					...
					$this->load->model('data_payment_method_model', 'main_model');
					$this->load->model('data_payment_method_status_model');
				}
			}
		} catch (Exception $ex) {
			$this->session->set_userdata($this->prefix . '_to_page', current_url());
	...
	function index()
	{
		...
		if ($this->cf->is_allowed_module($this->module, $prefix)) {
			...
			$data['status'] = $this->data_payment_method_status_model->search("");
			$this->load->view('data_payment_method/index', $data);
		} else {
			$this->session->set_userdata($this->prefix . '_to_page', current_url());
			redirect(base_url() . 'authentication');
		}
