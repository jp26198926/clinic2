...
class Data_result_set extends CI_Controller
{
	...
	protected $role_id;
	protected $module = "data_result_set";
	protected $module_description = "RESULT_SET";
	protected $page_name = "RESULT_SET";
		...
		try {
			...
			if ($ad) {
				...
				if (!isset($this->session->userdata[$this->prefix . '_logged_in'])) {
					...
					$this->load->model('data_result_set_model', 'main_model');
					$this->load->model('data_result_set_status_model');
	...
	function index()
	{
		...
		if ($this->cf->is_allowed_module($this->module, $prefix)) {
			...
			$data['status'] = $this->data_result_set_status_model->search("");
			$data['products'] = $this->data_product_model->search("");
			$data['categories'] = $this->data_category_model->search("");
			$this->load->view('data_result_set/index', $data);
		} else {
			$this->session->set_userdata($this->prefix . '_to_page', current_url());
			redirect(base_url() . 'authentication');
		}
	...
	function search_info_row()
	{
		...
		if ($id) {
			try {
				...
				$info = $this->main_model->search_by_row($id);
				if ($info) {
	...
	function add()
	{
		...
		if ($data_input["product_id"] && $data_input["result_label"]) {
			try {
	...
	function get_product_category(){
		...
		if ($product_id){
			try{
				$product = $this->data_product_model->get_product_category($product_id);
