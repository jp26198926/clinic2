...
<body class="no-skin">
    ...
    <div class="main-container ace-save-state" id="main-container">
        ...
        <?php
        $this->load->view('data_location/modal_new');
        $this->load->view('data_location/modal_modify');
        $this->load->view('data_location/modal_info');
        $this->load->view('data_location/modal_asearch');
    ...
    function populate_table(table_ref, result_data) {
        ...
        $.each(result_data, function(index, value) {
            ...
            dataSet.push([option, value.location, status, id, status_id]);
        });
    ...
    $(document).on("click", "#btn_search", function() {
        ...
        $.get("<?= base_url(); ?>data_location/search?search=" + search, function(data) {
            $("#loading").modal("hide");
    ...
    $(document).on("click", "#btn_asearch", function() {
        ...
        $("#modal_asearch").on("shown.bs.modal", function() {
            $("#txt_location_asearch").trigger("select", "focus");
        });
    });
    ...
    $(document).on("click", "#btn_asearch_start", function() {
        var location = $("#txt_location_asearch").val().trim();
        var status_id = $("#txt_status_asearch").val();

        $(".modal_error, .modal_button").hide();
        $(".modal_waiting").show();

        $.post("<?= base_url(); ?>data_location/advance_search", {
            location: location,
            status_id: status_id
        }, function(data) {
    ...
    $(document).on("click", "#btn_new", function() {
        ...
        $('#modal_new').on('shown.bs.modal', function() {
            $('#txt_location').trigger('select', 'focus');
        });
    });
    ...
    $(document).on("click", "#btn_save", function() {
        ...
        var location = $("#txt_location").val().trim();

        if (location) {
            $(".modal_error, .modal_button").hide();
            $(".modal_waiting").show();

            $.post("<?= base_url(); ?>data_location/add", {
                location: location
            }, function(data) {
    ...
    $(document).on("click", ".btn_modify", function(e) {
        ...
        if (id) {
            ...
            $.post("<?= base_url(); ?>data_location/search_info_row", {
                id: id
            }, function(data) {
                ...
                if (data.indexOf("<!DOCTYPE html>") > -1) {
                    ...
                    $("#txt_location_update").val(data.location);

                    $("#modal_modify").modal();

                    $("#modal_modify").on("shown.bs.modal", function() {
                        $("#txt_location_update").trigger("select", "focus");
                    });
                }
            });
        } else {
    ...
    $(document).on("click", "#btn_update", function() {
        ...
        if (id) {
            if (location) {
                ...
                $.post("<?= base_url(); ?>data_location/update", {
                    id: id,
                    location: location
                }, function(data) {
        ...
        if (id) {
            ...
            $.post("<?= base_url(); ?>data_location/search_info_row", {
                ...
                if (data.indexOf("<!DOCTYPE html>") > -1) {
                    ...
                    $("#txt_location_info").val(data.location);
        ...
        if (id) {
            ...
            bootbox.prompt({
                ...
                callback: function(result) {
                    ...
                    if (result !== null) {
                        if (result) {
                            $.post("<?= base_url(); ?>data_location/delete", {
                                id: id,
                                reason: result
                            }, function(data) {
                                if (data.indexOf("<!DOCTYPE html>") > -1) {
        ...
        if (id) {
            ...
            bootbox.confirm("Are you sure you want to ACTIVATE this data?", function(result) {
                if (result) {
                    ...
                    $.post("<?= base_url(); ?>data_location/activate", {
                        id: id
                    }, function(data) {
