...
<body class="no-skin">
	...
	<div class="main-container ace-save-state" id="main-container">
		...
		<?php
		$this->load->view('setup_common_status/modal_new');
		$this->load->view('setup_common_status/modal_modify');
		$this->load->view('setup_common_status/modal_asearch');
	...
	<script>
		...
		function table_buttons(table, id, status_id = 1) {
			...
			?>
				option += "<button id='" + id + "' class='btn_modify btn btn-xs btn-info fa fa-pencil' title='Modify' data-toggle='tooltip'></button> ";
			<?php
			}
			?>
		...
		function populate_table(table_ref, result_data) {
			...
			$.each(result_data, function(index, value) {
				var id = value.id;
				var status = value.status.toUpperCase();
				var status_id = 0;

				var option = table_buttons(table_ref, id, status_id);
				dataSet.push([option, status, id]);
			});
		...
		$(document).on("click", "#btn_search", function() {
			...
			$.get("<?= base_url(); ?>setup_common_status/search?search=" + search, function(data) {
		...
		$(document).on("click", "#btn_asearch", function() {
			...
			$("#modal_asearch").on("shown.bs.modal", function() {
				$("#txt_status_asearch").trigger("select", "focus");
			});
		});

		$(document).on("keypress", "#txt_status_asearch", function(e) {
			if (e.which == 13) {
		...
		$(document).on("click", "#btn_asearch_start", function() {
			var status = $("#txt_status_asearch").val().trim();


			$(".modal_error, .modal_button").hide();
			$(".modal_waiting").show();

			$.post("<?= base_url(); ?>setup_common_status/advance_search", {
				status: status
			}, function(data) {
		...
		$(document).on("click", "#btn_new", function() {
			...
			$('#modal_new').on('shown.bs.modal', function() {
				$('#txt_status').trigger('select', 'focus');
			});
		});

		$(document).on("keypress", "#txt_status", function(e) {
			if (e.which == 13) {
		...
		$(document).on("click", "#btn_save", function() {
			var status = $("#txt_status").val().trim();

			if (status) {
				$(".modal_error, .modal_button").hide();
				$(".modal_waiting").show();

				$.post("<?= base_url(); ?>setup_common_status/add", {
					status: status
				}, function(data) {
		...
		$(document).on("click", ".btn_modify", function(e) {
			...
			if (id) {
				...
				$.post("<?= base_url(); ?>setup_common_status/search_info_row", {
					id: id
				}, function(data) {
					...
					if (data.indexOf("<!DOCTYPE html>") > -1) {
						...
						$("#txt_status_update").val(data.status);

						$("#modal_modify").modal();

						$("#modal_modify").on("shown.bs.modal", function() {
							$("#txt_status_update").trigger("select", "focus");
						});
					}
				});
			} else {
		...
		$(document).on("keypress", "#txt_status_update", function(e) {
			if (e.which == 13) {
		...
		$(document).on("click", "#btn_update", function() {
			...
			var status = $("#txt_status_update").val();
			...
			if (id) {
				if (status) {
					...
					$.post("<?= base_url(); ?>setup_common_status/update", {
						id: id,
						status: status
					}, function(data) {
		...
		// 				$.post("<?= base_url(); ?>setup_common_status/update_status", {
		// 					id: id,
		// 					status_id: status_id
		// 				}, function(data) {
		...
		// 						bootbox.alert(data);
		// 					} else {
		// 						current_data[0] = table_buttons("#tbl_list", current_data[7], status_id)
