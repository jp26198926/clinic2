...
<body class="no-skin">
    ...
    <div class="main-container ace-save-state" id="main-container">
        ...
        <?php
        $this->load->view('data_payment_type/modal_new');
        $this->load->view('data_payment_type/modal_modify');
        $this->load->view('data_payment_type/modal_info');
        $this->load->view('data_payment_type/modal_asearch');
    ...
    function populate_table(table_ref, result_data) {
        ...
        $.each(result_data, function(index, value) {
            ...
            dataSet.push([option, value.payment_type, status, id, status_id]);
        });
    ...
    $(document).on("click", "#btn_search", function() {
        ...
        $.get("<?= base_url(); ?>data_payment_type/search?search=" + search, function(data) {
            $("#loading").modal("hide");
    ...
    $(document).on("click", "#btn_asearch", function() {
        ...
        $("#modal_asearch").on("shown.bs.modal", function() {
            $("#txt_payment_type_asearch").trigger("select", "focus");
        });
    });
    ...
    $(document).on("click", "#btn_asearch_start", function() {
        var payment_type = $("#txt_payment_type_asearch").val().trim();
        var status_id = $("#txt_status_asearch").val();

        $(".modal_error, .modal_button").hide();
        $(".modal_waiting").show();

        $.post("<?= base_url(); ?>data_payment_type/advance_search", {
    ...
    $(document).on("click", "#btn_new", function() {
        ...
        $('#modal_new').on('shown.bs.modal', function() {
            $('#txt_payment_type').trigger('select', 'focus');
        });
    });
    ...
    $(document).on("click", "#btn_save", function() {
        ...
        var payment_type = $("#txt_payment_type").val().trim();

        if (payment_type) {
            $(".modal_error, .modal_button").hide();
            $(".modal_waiting").show();

            $.post("<?= base_url(); ?>data_payment_type/add", {
                payment_type: payment_type
            }, function(data) {
    ...
    $(document).on("click", ".btn_modify", function(e) {
        ...
        if (id) {
            ...
            $.post("<?= base_url(); ?>data_payment_type/search_info_row", {
                id: id
            }, function(data) {
                ...
                if (data.indexOf("<!DOCTYPE html>") > -1) {
                    ...
                    $("#txt_payment_type_update").val(data.payment_type);

                    $("#modal_modify").modal();

                    $("#modal_modify").on("shown.bs.modal", function() {
                        $("#txt_payment_type_update").trigger("select", "focus");
                    });
                }
            });
        } else {
    ...
    $(document).on("click", "#btn_update", function() {
        ...
        if (id) {
            if (payment_type) {
                ...
                $.post("<?= base_url(); ?>data_payment_type/update", {
                    id: id,
                    payment_type: payment_type
                }, function(data) {
        ...
        if (id) {
            ...
            $.post("<?= base_url(); ?>data_payment_type/search_info_row", {
                ...
                if (data.indexOf("<!DOCTYPE html>") > -1) {
                    ...
                    $("#txt_payment_type_info").val(data.payment_type);
        ...
        if (id) {
            ...
            bootbox.prompt({
                ...
                callback: function(result) {
                    ...
                    if (result !== null) {
                        if (result) {
                            $.post("<?= base_url(); ?>data_payment_type/delete", {
                                id: id,
                                reason: result
                            }, function(data) {
                                if (data.indexOf("<!DOCTYPE html>") > -1) {
        ...
        if (id) {
            ...
            bootbox.confirm("Are you sure you want to ACTIVATE this data?", function(result) {
                if (result) {
                    ...
                    $.post("<?= base_url(); ?>data_payment_type/activate", {
                        id: id
                    }, function(data) {
