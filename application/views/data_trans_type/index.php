...
<body class="no-skin">
    ...
    <div class="main-container ace-save-state" id="main-container">
        ...
        <?php
		$this->load->view('data_trans_type/modal_new');
		$this->load->view('data_trans_type/modal_modify');
		$this->load->view('data_trans_type/modal_info');
		$this->load->view('data_trans_type/modal_asearch');
    ...
    function populate_table(table_ref, result_data) {
        ...
        $.each(result_data, function(index, value) {
            ...
            dataSet.push([option, value.trans_type, status, id, status_id]);
        });
    ...
    $(document).on("click", "#btn_search", function() {
        ...
        $.get("<?= base_url(); ?>data_trans_type/search?search=" + search, function(data) {
            $("#loading").modal("hide");

            if (data.indexOf("<!DOCTYPE html>") > -1) {
                alert("Error: Session Time-Out, You must login again to continue.");
                trans_type.reload(true);
    ...
    $(document).on("click", "#btn_asearch", function() {
        ...
        $("#modal_asearch").on("shown.bs.modal", function() {
            $("#txt_trans_type_asearch").trigger("select", "focus");
        });
    });
    ...
    $(document).on("click", "#btn_asearch_start", function() {
        var trans_type = $("#txt_trans_type_asearch").val().trim();
        var status_id = $("#txt_status_asearch").val();

        $(".modal_error, .modal_button").hide();
        $(".modal_waiting").show();

        $.post("<?= base_url(); ?>data_trans_type/advance_search", {
    ...
    $(document).on("click", "#btn_new", function() {
        ...
        $('#modal_new').on('shown.bs.modal', function() {
            $('#txt_trans_type').trigger('select', 'focus');
        });
    });
    ...
    $(document).on("click", "#btn_save", function() {
        ...
        var trans_type = $("#txt_trans_type").val().trim();

        if (trans_type) {
            $(".modal_error, .modal_button").hide();
            $(".modal_waiting").show();

            $.post("<?= base_url(); ?>data_trans_type/add", {
                trans_type: trans_type
            }, function(data) {
    ...
    $(document).on("click", ".btn_modify", function(e) {
        ...
        if (id) {
            ...
            $.post("<?= base_url(); ?>data_trans_type/search_info_row", {
                id: id
            }, function(data) {
                ...
                if (data.indexOf("<!DOCTYPE html>") > -1) {
                    ...
                    $("#txt_trans_type_update").val(data.name);

                    $("#modal_modify").modal();

                    $("#modal_modify").on("shown.bs.modal", function() {
                        $("#txt_trans_type_update").trigger("select", "focus");
                    });
                }
            });
        } else {
    ...
    $(document).on("click", "#btn_update", function() {
        ...
        var trans_type = $("#txt_trans_type_update").val();
        ...
        if (id) {
            if (trans_type) {
                ...
                $.post("<?= base_url(); ?>data_trans_type/update", {
                    id: id,
                    trans_type: trans_type
                }, function(data) {
        ...
        if (id) {
            ...
            $.post("<?= base_url(); ?>data_trans_type/search_info_row", {
                ...
                if (data.indexOf("<!DOCTYPE html>") > -1) {
                    ...
                    $("#txt_trans_type_info").val(data.name);
        ...
        if (id) {
            ...
            bootbox.prompt({
                ...
                callback: function(result) {
                    ...
                    if (result !== null) {
                        if (result) {
                            $.post("<?= base_url(); ?>data_trans_type/delete", {
                                id: id,
                                reason: result
                            }, function(data) {
                                if (data.indexOf("<!DOCTYPE html>") > -1) {
        ...
        if (id) {
            ...
            bootbox.confirm("Are you sure you want to ACTIVATE this data?", function(result) {
                if (result) {
                    ...
                    $.post("<?= base_url(); ?>data_trans_type/activate", {
                        id: id
                    }, function(data) {
