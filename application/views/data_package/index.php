...
<body class="no-skin">
    ...
    <div class="main-container ace-save-state" id="main-container">
        ...
        <?php
		$this->load->view('data_package/modal_new');
		$this->load->view('data_package/modal_modify');
		$this->load->view('data_package/modal_info');
		$this->load->view('data_package/modal_asearch');
    ...
    function table_buttons(id, status_id = 1) {
        var option = "";
        option += "<a href='#' id='" + id +
            "' class='btn_items btn btn-xs btn-success fa fa-list  ' title='Item List'   data-toggle='tooltip'></a> ";
        option += "<a href='#' id='" + id +
    ...
    function populate_table(table_ref, result_data) {
        ...
        $.each(result_data, function(index, value) {
            ...
            dataSet.push([option, value.package, status, id, status_id]);
        });
    ...
    $(document).on("click", "#btn_search", function() {
        ...
        $.get("<?= base_url(); ?>data_package/search?search=" + search, function(data) {
            $("#loading").modal("hide");
    ...
    $(document).on("click", "#btn_asearch", function() {
        ...
        $("#modal_asearch").on("shown.bs.modal", function() {
            $("#txt_package_asearch").trigger("select", "focus");
        });
    });
    ...
    $(document).on("click", "#btn_asearch_start", function() {
        var package = $("#txt_package_asearch").val().trim();
        var status_id = $("#txt_status_asearch").val();

        $(".modal_error, .modal_button").hide();
        $(".modal_waiting").show();

        $.post("<?= base_url(); ?>data_package/advance_search", {
            package: package,
            status_id: status_id
        }, function(data) {
    ...
    $(document).on("click", "#btn_new", function() {
        ...
        $('#modal_new').on('shown.bs.modal', function() {
            $('#txt_package').trigger('select', 'focus');
        });
    });
    ...
    $(document).on("click", "#btn_save", function() {
        ...
        var package = $("#txt_package").val().trim();

        if (package) {
            $(".modal_error, .modal_button").hide();
            $(".modal_waiting").show();

            $.post("<?= base_url(); ?>data_package/add", {
                package: package
            }, function(data) {
    ...
    $(document).on("click", ".btn_modify", function(e) {
        ...
        if (id) {
            ...
            $.post("<?= base_url(); ?>data_package/search_info_row", {
                id: id
            }, function(data) {
                ...
                if (data.indexOf("<!DOCTYPE html>") > -1) {
                    ...
                    $("#txt_package_update").val(data.package);

                    $("#modal_modify").modal();

                    $("#modal_modify").on("shown.bs.modal", function() {
                        $("#txt_package_update").trigger("select", "focus");
                    });
                }
            });
        } else {
    ...
    $(document).on("click", "#btn_update", function() {
        ...
        var package = $("#txt_package_update").val();
        ...
        if (id) {
            if (package) {
                ...
                $.post("<?= base_url(); ?>data_package/update", {
                    id: id,
                    package: package
                }, function(data) {
        ...
        if (id) {
            ...
            $.post("<?= base_url(); ?>data_package/search_info_row", {
                ...
                if (data.indexOf("<!DOCTYPE html>") > -1) {
                    ...
                    $("#txt_package_info").val(data.package);
        ...
        if (id) {
            ...
            bootbox.prompt({
                ...
                callback: function(result) {
                    ...
                    if (result !== null) {
                        if (result) {
                            $.post("<?= base_url(); ?>data_package/delete", {
                                id: id,
                                reason: result
                            }, function(data) {
                                if (data.indexOf("<!DOCTYPE html>") > -1) {
        ...
        if (id) {
            ...
            bootbox.confirm("Are you sure you want to ACTIVATE this data?", function(result) {
                if (result) {
                    ...
                    $.post("<?= base_url(); ?>data_package/activate", {
                        id: id
                    }, function(data) {
