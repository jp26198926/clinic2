...
<body class="no-skin">
    ...
    <div class="main-container ace-save-state" id="main-container">
        ...
        <?php
		$this->load->view('item_category/modal_new');
		$this->load->view('item_category/modal_modify');
		$this->load->view('item_category/modal_info');
		$this->load->view('item_category/modal_asearch');
    ...
    function populate_table(table_ref, result_data) {
        ...
        $.each(result_data, function(index, value) {
            ...
            dataSet.push([option, category, status, id, status_id]);
        });
    ...
    $(document).on("click", "#btn_search", function() {
        ...
        $.get("<?= base_url(); ?>item_category/search?search=" + search, function(data) {
            $("#loading").modal("hide");
    ...
    $(document).on("click", "#btn_asearch", function() {
        ...
        $("#modal_asearch").on("shown.bs.modal", function() {
            $("#txt_category_asearch").trigger("select", "focus");
        });
    });
    ...
    $(document).on("click", "#btn_asearch_start", function() {
        var category = $("#txt_category_asearch").val().trim();
        var status_id = $("#txt_status_asearch").val();

        $(".modal_error, .modal_button").hide();
        $(".modal_waiting").show();

        $.post("<?= base_url(); ?>item_category/advance_search", {
            category: category,
            status_id: status_id
        }, function(data) {
    ...
    $(document).on("click", "#btn_new", function() {
        ...
        $('#modal_new').on('shown.bs.modal', function() {
            $('#txt_category').trigger('select', 'focus');
        });
    });
    ...
    $(document).on("click", "#btn_save", function() {
        ...
        if (category) {
            ...
            $.post("<?= base_url(); ?>item_category/add", {
                category: category
            }, function(data) {
    ...
    $(document).on("click", ".btn_modify", function(e) {
        ...
        if (id) {
            ...
            $.post("<?= base_url(); ?>item_category/search_info_row", {
                id: id
            }, function(data) {
                ...
                if (data.indexOf("<!DOCTYPE html>") > -1) {
                    ...
                    $("#txt_category_update").val(data.category);

                    $("#modal_modify").modal();

                    $("#modal_modify").on("shown.bs.modal", function() {
                        $("#txt_category_update").trigger("select", "focus");
                    });
                }
            });
        } else {
    ...
    $(document).on("click", "#btn_update", function() {
        ...
        if (id) {
            if (category) {
                ...
                $.post("<?= base_url(); ?>item_category/update", {
                    id: id,
                    category: category
                }, function(data) {
        ...
        if (id) {
            ...
            $.post("<?= base_url(); ?>item_category/search_info_row", {
                ...
                if (data.indexOf("<!DOCTYPE html>") > -1) {
                    ...
                    $("#txt_category_info").val(data.category);
        ...
        if (id) {
            ...
            bootbox.prompt({
                ...
                callback: function(result) {
                    ...
                    if (result !== null) {
                        if (result) {
                            $.post("<?= base_url(); ?>item_category/delete", {
                                id: id,
                                reason: result
                            }, function(data) {
                                if (data.indexOf("<!DOCTYPE html>") > -1) {
        ...
        if (id) {
            ...
            bootbox.confirm("Are you sure you want to ACTIVATE this data?", function(result) {
                if (result) {
                    ...
                    $.post("<?= base_url(); ?>item_category/activate", {
                        id: id
                    }, function(data) {
