...
<body class="no-skin">
    ...
    <div class="main-container ace-save-state" id="main-container">
        ...
        <?php
		$this->load->view('data_result_set/modal_new');
		$this->load->view('data_result_set/modal_modify');
		$this->load->view('data_result_set/modal_info');
		$this->load->view('data_result_set/modal_asearch');
    ...
    $(document).on("click", "#btn_search", function() {
        ...
        $.get("<?= base_url(); ?>data_result_set/search?search=" + search, function(data) {
            $("#loading").modal("hide");
    ...
    $(document).on("click", "#btn_asearch_start", function() {
        ...
        $.post("<?= base_url(); ?>data_result_set/advance_search", {
            code: code,
            name: name,
            uom_id: uom_id,
    ...
    $(document).on("change", "#txt_product_id", function() {
        let product_id = $(this).val();

        if (product_id) {
            $.post("<?= base_url(); ?>data_result_set/get_product_category", {
    ...
    $(document).on("click", "#btn_new", function() {
        ...
        $('#modal_new').on('shown.bs.modal', function() {
            ...
            $("#txt_product_id").trigger("focus, change");
        });
    });
    ...
    $(document).on("click", "#btn_save", function() {
        ...
        var result_label = $("#txt_result_label").val();
        var reference = $("#txt_reference").val();
        var unit = $("#txt_unit").val();


        if (product_id && result_label) {
            $("#modal_new .modal_error, #modal_new .modal_button").hide();
            $("#modal_new .modal_waiting").show();

            $.post("<?= base_url(); ?>data_result_set/add", {
                product_id: product_id,
    ...
    $(document).on("click", ".btn_modify", function(e) {
        ...
        if (id) {
            ...
            $.post("<?= base_url(); ?>data_result_set/search_info_row", {
                id: id
            }, function(data) {
                ...
                if (data.indexOf("<!DOCTYPE html>") > -1) {
                    ...
                    $("#txt_product_id_update").val(data.product_id);
                    $("#txt_category_id_update").val(data.category_id);
                    $("#txt_result_label_update").val(data.result_label);
                    $("#txt_reference_update").val(data.reference);
                    $("#txt_unit_update").val(data.unit);

                    $("#modal_modify").modal();

                    $("#modal_modify").on("shown.bs.modal", function() {
                        $("#txt_product_id_update").trigger("select", "focus");
                    });
                }
            });
        } else {
    ...
    $(document).on("click", "#btn_update", function() {
        ...
        var product_id = $("#txt_product_id_update").val();
        var product = $("#txt_product_id_update option:selected").text();
        var category = $("#txt_category_id_update option:selected").text();
        var result_label = $("#txt_result_label_update").val();
        var reference = $("#txt_reference_update").val();
        var unit = $("#txt_unit_update").val();

        if (id) {
            if (product_id && result_label) {
                $("#modal_modify .modal_error, #modal_new .modal_button").hide();
                $("#modal_modify .modal_waiting").show();

                $.post("<?= base_url(); ?>data_result_set/update", {
                    id: id,
                    product_id: product_id,
                    result_label: result_label,
                    reference: reference,
                    ...
                    if (data.indexOf("<!DOCTYPE html>") > -1) {
                        ...
                        current_data[1] = category;
                        current_data[2] = product;
                        current_data[3] = result_label;
                        current_data[4] = reference;
                        current_data[5] = unit;
        ...
        if (id) {
            ...
            $.post("<?= base_url(); ?>data_result_set/search_info_row", {
                ...
                if (data.indexOf("<!DOCTYPE html>") > -1) {
                    ...
                    $("#txt_product_id_info").val(data.product_id);
                    $("#txt_category_id_info").val(data.category_id);
                    $("#txt_result_label_info").val(data.result_label);
                    $("#txt_reference_info").val(data.reference);
                    $("#txt_unit_info").val(data.unit);

                    $("#modal_info").modal();
                    $(".txt_field_info").prop("disabled", "disabled");
                }
            });
        } else {
            bootbox.alert("Error: Critical Error Encountered!");
        ...
        if (id) {
            ...
            bootbox.prompt({
                ...
                callback: function(result) {
                    ...
                    if (result !== null) {
                        if (result) {
                            $.post("<?= base_url(); ?>data_result_set/delete", {
                                id: id,
                                reason: result
                            }, function(data) {
                                if (data.indexOf("<!DOCTYPE html>") > -1) {
        ...
        if (id) {
            ...
            bootbox.confirm("Are you sure you want to ACTIVATE this data?", function(result) {
                if (result) {
                    ...
                    $.post("<?= base_url(); ?>data_result_set/activate", {
                        id: id
                    }, function(data) {
