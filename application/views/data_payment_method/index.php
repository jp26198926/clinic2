...
<body class="no-skin">
    ...
    <div class="main-container ace-save-state" id="main-container">
                                ...
                                <div class="col-xs-12">
                                    ...
                                    <div>
                                        <table id="tbl_list"
                                            ...
                                            <tbody>
                                                <tr>
                                                    <td align='center' colspan='3'>Use search button to display record
                                                    </td>
                                                </tr>
                                            </tbody>
                                        </table>
        ...
        <?php
        $this->load->view('data_payment_method/modal_new');
        $this->load->view('data_payment_method/modal_modify');
        $this->load->view('data_payment_method/modal_info');
        $this->load->view('data_payment_method/modal_asearch');
    ...
    function populate_table(table_ref, result_data) {
        ...
        $.each(result_data, function(index, value) {
            ...
            dataSet.push([option, value.payment_method, status, id, status_id]);
        });
    ...
    $(document).on("click", "#btn_search", function() {
        ...
        $.get("<?= base_url(); ?>data_payment_method/search?search=" + search, function(data) {
            $("#loading").modal("hide");
    ...
    $(document).on("click", "#btn_asearch", function() {
        ...
        $("#modal_asearch").on("shown.bs.modal", function() {
            $("#txt_payment_method_asearch").trigger("select", "focus");
        });
    });
    ...
    $(document).on("click", "#btn_asearch_start", function() {
        var payment_method = $("#txt_payment_method_asearch").val().trim();
        var status_id = $("#txt_status_asearch").val();

        $(".modal_error, .modal_button").hide();
        $(".modal_waiting").show();

        $.post("<?= base_url(); ?>data_payment_method/advance_search", {
    ...
    $(document).on("click", "#btn_new", function() {
        ...
        $('#modal_new').on('shown.bs.modal', function() {
            $('#txt_payment_method').trigger('select', 'focus');
        });
    });
    ...
    $(document).on("click", "#btn_save", function() {
        ...
        var payment_method = $("#txt_payment_method").val().trim();

        if (payment_method) {
            $(".modal_error, .modal_button").hide();
            $(".modal_waiting").show();

            $.post("<?= base_url(); ?>data_payment_method/add", {
                payment_method: payment_method
            }, function(data) {
    ...
    $(document).on("click", ".btn_modify", function(e) {
        ...
        if (id) {
            ...
            $.post("<?= base_url(); ?>data_payment_method/search_info_row", {
                id: id
            }, function(data) {
                ...
                if (data.indexOf("<!DOCTYPE html>") > -1) {
                    ...
                    $("#txt_payment_method_update").val(data.payment_method);

                    $("#modal_modify").modal();

                    $("#modal_modify").on("shown.bs.modal", function() {
                        $("#txt_payment_method_update").trigger("select", "focus");
                    });
                }
            });
        } else {
    ...
    $(document).on("click", "#btn_update", function() {
        ...
        var payment_method = $("#txt_payment_method_update").val();
        ...
        if (id) {
            if (payment_method) {
                ...
                $.post("<?= base_url(); ?>data_payment_method/update", {
                    id: id,
                    payment_method: payment_method
                }, function(data) {
        ...
        if (id) {
            ...
            $.post("<?= base_url(); ?>data_payment_method/search_info_row", {
                ...
                if (data.indexOf("<!DOCTYPE html>") > -1) {
                    ...
                    $("#txt_payment_method_info").val(data.payment_method);
        ...
        if (id) {
            ...
            bootbox.prompt({
                ...
                callback: function(result) {
                    ...
                    if (result !== null) {
                        if (result) {
                            $.post("<?= base_url(); ?>data_payment_method/delete", {
                                id: id,
                                reason: result
                            }, function(data) {
                                if (data.indexOf("<!DOCTYPE html>") > -1) {
        ...
        if (id) {
            ...
            bootbox.confirm("Are you sure you want to ACTIVATE this data?", function(result) {
                if (result) {
                    ...
                    $.post("<?= base_url(); ?>data_payment_method/activate", {
                        id: id
                    }, function(data) {
